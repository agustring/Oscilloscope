...
Item {
 ...
 Item { id: dial; width: 62; height: 62; anchors.horizontalCenter: parent.horizontalCenter; anchors.top: parent.top; anchors.topMargin: 16
  ...
  MouseArea { id:mouse; anchors.fill:parent; hoverEnabled:true; property real startY
   onPressed:{root.forceActiveFocus();startY=mouseY}
   onPositionChanged: if(pressed&&Math.abs(mouseY-startY)>7){var up=mouseY<startY;root.angle+=up?12:-12;up?root.increment((modifiers&Qt.ShiftModifier)!==0):root.decrement((modifiers&Qt.ShiftModifier)!==0);startY=mouseY}
   onClicked:if(root.pushEnabled)root.pushed()
