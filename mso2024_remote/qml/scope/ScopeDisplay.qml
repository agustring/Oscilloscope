...
import "../menus"
import "../components"
Rectangle { id:root; color:ScopeTheme.display; border.color:"#65706b"; border.width:2; property var traces:scope.waveforms
 ...
 Canvas { id:wave; anchors.fill:parent; anchors.rightMargin:scope.sideMenuVisible?148:0; anchors.bottomMargin:scope.menuContext?68:0
  onPaint:{var c=getContext("2d"),w=width,h=height;c.reset();c.fillStyle=ScopeTheme.display;c.fillRect(0,0,w,h);c.lineWidth=1
   ...
   for(var ch=0;ch<4;ch++)if(scope.channelEnabled(ch+1)){var a=root.traces[ch];if(!a||a.length<2)continue;c.strokeStyle=ScopeTheme.channels[ch];c.lineWidth=1.7;c.beginPath();var s=Math.max(scope.channelScale(ch+1),1e-9);for(var i=0;i<a.length;i++){var px=i*w/(a.length-1),py=h/2-a[i]*h/(8*s);i?c.lineTo(px,py):c.moveTo(px,py)}c.stroke()}
   var ref=scope.referenceWaveform;if(scope.referenceVisible&&ref&&ref.length>1){c.strokeStyle="#d7d7d0";c.lineWidth=1.2;c.setLineDash([5,3]);c.beginPath();var rs=Math.max(scope.channelScale(scope.selectedChannel),1e-9);for(var ri=0;ri<ref.length;ri++){var rx=ri*w/(ref.length-1),ry=h/2-ref[ri]*h/(8*rs);ri?c.lineTo(rx,ry):c.moveTo(rx,ry)}c.stroke();c.setLineDash([])}
  }
  MouseArea { anchors.fill:parent; acceptedButtons:Qt.LeftButton|Qt.RightButton; property real lastY; property real lastX
   onPressed:{lastY=mouseY;lastX=mouseX;if(mouse.button===Qt.RightButton){contextMenu.x=Math.min(mouseX,wave.width-contextMenu.width);contextMenu.y=Math.min(mouseY,wave.height-contextMenu.height);contextMenu.visible=true;scope.selectChannel(scope.selectedChannel)}}
   onPositionChanged:if(pressed){var dx=mouseX-lastX,dy=mouseY-lastY;if(Math.abs(dx)>Math.abs(dy)&&Math.abs(dx)>5){scope.adjustHorizontalDelay(-dx*10/width);lastX=mouseX;lastY=mouseY}else if(Math.abs(dy)>8){scope.adjustChannelPosition(scope.selectedChannel,dy<0?1:-1,(modifiers&Qt.ShiftModifier)!==0);lastX=mouseX;lastY=mouseY}}
 ...
 Rectangle { id:contextMenu; visible:false; width:180;height:210;color:"#151c1c";border.color:ScopeTheme.channels[scope.selectedChannel-1];z:15
  Column { anchors.fill:parent;anchors.margins:6;spacing:4
   ...
   ScopeButton { width:parent.width;text:scope.channelEnabled(scope.selectedChannel)?"DISABLE CHANNEL":"ENABLE CHANNEL";onClicked:{scope.pressChannel(scope.selectedChannel);contextMenu.visible=false} }
 ...
 Repeater { model:[scope.cursorY1,scope.cursorY2]
  Item { visible:scope.cursorFunction==="HBARS"||scope.cursorFunction==="SCREEN"; height:11; width:wave.width; x:0; y:Math.max(0,Math.min(wave.height-11,wave.height*(0.5-modelData/(8*scope.channelScale(scope.selectedChannel)))-5))
   ...
   MouseArea { anchors.fill:parent; cursorShape:Qt.SizeVerCursor; onPositionChanged:if(pressed){var p=mapToItem(wave,mouseX,mouseY);scope.setCursorPosition("HBARS",index+1,(0.5-p.y/wave.height)*8*scope.channelScale(scope.selectedChannel))} }
  }
 }
 Repeater { model:4
  Item { visible:scope.channelEnabled(index+1); width:18;height:18;x:0;y:Math.max(12,Math.min(wave.height-20,wave.height/2-scope.channelPosition(index+1)*wave.height/8-9))
 ...
 Rectangle { visible:scope.cursorFunction!=="OFF"; width:190; height:scope.cursorFunction==="SCREEN"?55:36; anchors.right:side.left; anchors.bottom:status.top; anchors.margins:5; color:"#111817"; border.color:ScopeTheme.warning
  ...
  Text { anchors.fill:parent; anchors.margins:5; color:ScopeTheme.warning; font.pixelSize:10
   text:(scope.cursorFunction==="VBARS"||scope.cursorFunction==="WAVEFORM"||scope.cursorFunction==="SCREEN"?("Δt  "+root.eng(parent.dt,"s")+"    1/Δt  "+(parent.dt>0?root.eng(1/parent.dt,"Hz"):"—")):"")+(scope.cursorFunction==="SCREEN"?"\n":"")+(scope.cursorFunction==="HBARS"||scope.cursorFunction==="SCREEN"?("ΔV  "+root.eng(parent.dv,"V")):"")
  }
 }
 Item { width:18;height:18;x:wave.width-18;y:Math.max(12,Math.min(wave.height-20,wave.height/2-scope.triggerLevel*wave.height/(8*scope.channelScale(scope.selectedChannel))-9))
  ...
  MouseArea { anchors.fill:parent; cursorShape:Qt.SizeVerCursor; onDoubleClicked:scope.centerTrigger(); onPositionChanged:if(pressed){var p=mapToItem(wave,mouseX,mouseY);scope.setTriggerLevel((wave.height/2-p.y)*8*scope.channelScale(scope.selectedChannel)/wave.height)} }
 }
 Row { anchors.left:parent.left; anchors.bottom:menu.top; anchors.margins:5; spacing:10
  Repeater { model:4; Text { visible:scope.channelEnabled(index+1); text:"CH"+(index+1)+"  "+scope.channelScale(index+1).toPrecision(2)+" V/div"; color:ScopeTheme.channels[index]; font.pixelSize:11; font.bold:true } }
 }
 ...
 Text { id:status; anchors.right:side.left; anchors.bottom:menu.top; anchors.margins:5; text:"H  "+root.eng(scope.timeScale,"s/div")+"   Delay  "+root.eng(scope.horizontalDelay,"s")+"   Trig "+scope.triggerSource+"  "+scope.triggerLevel.toFixed(2)+" V   "+scope.waveformPointCount+" pts"; color:"#9fc4ca"; font.pixelSize:10; font.bold:true }
